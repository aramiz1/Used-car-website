	...
	switch($_GET["action"]) {
		case "add":
			...
			if(!empty($_POST["quantity"])) {
				$productByCode = $db_handle->runQuery("SELECT * FROM finaltable WHERE wheelcode='" . $_GET["wheelcode"] . "'");
				$itemArray = array($productByCode[0]["wheelcode"]=>array('wheelname'=>$productByCode[0]["wheelname"], 'wheelcode'=>$productByCode[0]["wheelcode"], 'quantity'=>$_POST["quantity"], 'price'=>$productByCode[0]["wheelprice"], ));
				
				$_SESSION["wheelSelected"] = $productByCode[0]["wheelname"];
				
				if(!empty($_SESSION["cart_item"])) {
					if(in_array($productByCode[0]["wheelcode"],array_keys($_SESSION["cart_item"]))) {
						foreach($_SESSION["cart_item"] as $k => $v) {
								if($productByCode[0]["wheelcode"] == $k) {
	...
	<body>
		...
		<div class="container">
							...
							foreach ($_SESSION["cart_item"] as $item){
								...
								?>
										...
										<td style="text-align:right;"><?php echo "$ ". number_format($item_price,2); ?></td>
																			<td style="text-align:center;"><a href="windows.php?action=remove&wheelcode=<?php echo $item["windowcode"]; 
			...
			<div id="product-grid">
				...
				<li><a href="wheels.php" class = "active">Wheels</a></li>
				...
				?>
					<div class="product-item">
						<form method="post" action="wheels.php?action=add&wheelcode=<?php echo $product_array[$key]["wheelcode"]; ?>">
						
						
										
						<div class="product-tile-footer">
						<img src="<?php echo  "wheelcode".$product_array[$key]["wheelcode"].".PNG"; ?>" class="cart-item-image">
						<div class="product-title"><?php echo $product_array[$key]["wheelname"]; ?></div>
						<div class="product-price"><?php echo "$".$product_array[$key]["wheelprice"]; ?></div>
