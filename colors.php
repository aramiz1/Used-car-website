	...
	switch($_GET["action"]) {
		case "add":
			...
			if(!empty($_POST["quantity"])) {
				$productByCode = $db_handle->runQuery("SELECT * FROM finaltable WHERE colorcode='" . $_GET["colorcode"] . "'");
				$itemArray = array($productByCode[0]["colorcode"]=>array('colorname'=>$productByCode[0]["colorname"], 'colorcode'=>$productByCode[0]["colorcode"], 'quantity'=>$_POST["quantity"], 'price'=>$productByCode[0]["colorprice"], ));
				
				$_SESSION["colorSelected"] = $productByCode[0]["colorname"];
				
				if(!empty($_SESSION["cart_item"])) {
					if(in_array($productByCode[0]["colorcode"],array_keys($_SESSION["cart_item"]))) {
						foreach($_SESSION["cart_item"] as $k => $v) {
								if($productByCode[0]["colorcode"] == $k) {
	...
	<body>
		...
		<div class="container">
			<div id="shopping-cart">
				...
				<a id="btnEmpty" href="colors.php?action=empty">Empty Cart</a>
				<?php
							...
							foreach ($_SESSION["cart_item"] as $item){
								...
								?>
										...
										<td style="text-align:right;"><?php echo "$ ". number_format($item_price,2); ?></td>
																			<td style="text-align:center;"><a href="windows.php?action=remove&colorcode=<?php echo $item["windowcode"]; 
			...
			<div id="product-grid">
				...
				<li><a href="colors.php" class = "active">Colors</a></li>
				</ul>
				...
				?>
					<div class="product-item">
						<form method="post" action="colors.php?action=add&colorcode=<?php echo $product_array[$key]["colorcode"]; ?>">
						
						
										
						<div class="product-tile-footer">
						<img src="<?php echo  "colorcode".$product_array[$key]["colorcode"].".PNG"; ?>" class="cart-item-image">
						<div class="product-title"><?php echo $product_array[$key]["colorname"]; ?></div>
						<div class="product-price"><?php echo "$".$product_array[$key]["colorprice"]; ?></div>
